...
public class AmmoCrateDelayUpgrade : MonoBehaviour  {
	...
	void Start () {
		upgradesBought = PlayerPrefs.GetInt ("subtractedTimeA");
	...
	public void BuyUpgrade()
	{
		if (prices [nextUpgrade] <= PlayerPrefs.GetInt ("Coins") && nextUpgrade <= boughtDots.Length) {
			...
			PlayerPrefs.SetInt ("subtractedTimeA", PlayerPrefs.GetInt ("subtractedTimeA") + 1);
