...
public class MaxHeartUpgrade : MonoBehaviour {
	...
	void Start () {
		upgradesBought = PlayerPrefs.GetInt ("AddedHearts");
	...
	public void BuyUpgrade()
	{
		if (prices [nextUpgrade] <= PlayerPrefs.GetInt ("Coins") && nextUpgrade <= boughtDots.Length) {
			...
			PlayerPrefs.SetInt ("AddedHearts", PlayerPrefs.GetInt ("AddedHearts") + 1);
