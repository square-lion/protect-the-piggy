...
public class StartingHeartsUpgrade : MonoBehaviour {
	...
	void Start () {
		upgradesBought = PlayerPrefs.GetInt ("AddedStartHearts");
	...
	void Update () {
		...
		if (upgradesBought >= 5) {
			boughtDots [4].SetActive (true);
		} else {
			boughtDots [4].SetActive (false);
		}
	...
	public void BuyUpgrade()
	{
		if (prices [nextUpgrade] <= PlayerPrefs.GetInt ("Coins") && nextUpgrade <= boughtDots.Length) {
			...
			PlayerPrefs.SetInt ("AddedStartHearts", PlayerPrefs.GetInt ("AddedStartHearts") + 1);
